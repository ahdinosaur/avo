...
use crate::{Flow, FlowMeta, NodeId};
...
pub fn compute_epochs<Node>(tree: Flow<Node>) -> Result<Vec<Vec<Node>>, EpochError>
where
    Node: Clone,
{
    #[derive(Debug)]
    ...
    fn collect_recursive<Node>(
        ...
        match tree {
            Flow::Branch { children, meta } => {
                let FlowMeta { id, before, after } = meta;
